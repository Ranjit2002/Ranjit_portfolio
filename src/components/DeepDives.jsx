...
export default function DeepDives({ selectedTab = 'github', setSelectedTab, theme }) {
  ...
  const topics = {
    github: {
      ...
      cards: [
        {
          tag: 'INIT',
          icon: <GitBranch className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: 'PUSH',
          icon: <GitPullRequest className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: 'MRGE',
          icon: <GitMerge className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
    ...
    problem: {
      ...
      cards: [
        {
          tag: '01',
          icon: <Search className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: '02',
          icon: <Play className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: '03',
          icon: <Bug className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
    ...
    responsive: {
      ...
      cards: [
        {
          tag: '01',
          icon: <Smartphone className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: '02',
          icon: <LayoutGrid className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
        ...
        {
          tag: '03',
          icon: <Gauge className="w-8 h-8 text-amber-500 dark:text-amber-400" />,
  ...
  return (
    <section id="deep-dives" className="relative py-24 md:py-32 overflow-hidden border-t border-zinc-200/80 dark:border-zinc-900/60 transition-colors">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] bg-gradient-to-r from-amber-400/10 via-yellow-300/5 to-transparent dark:from-amber-500/10 dark:via-yellow-400/5 blur-[160px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex justify-center mb-8">
          <div className="inline-flex p-1.5 bg-white/90 dark:bg-zinc-950/80 border border-zinc-200/90 dark:border-zinc-800 rounded-2xl md:rounded-full backdrop-blur-xl flex-wrap justify-center gap-1.5 shadow-md dark:shadow-none transition-colors">
            <button
              ...
              className={`px-6 py-3 rounded-full text-xs md:text-sm font-bold uppercase tracking-wider transition-all duration-300 ${
                currentTab === 'github'
                  ? 'bg-gradient-to-r from-amber-400 to-yellow-500 text-black shadow-lg shadow-amber-400/30 font-bold'
        ...
        <div className="max-w-2xl mx-auto mb-12 p-3.5 rounded-2xl bg-zinc-900 dark:bg-zinc-950 border border-zinc-700/60 dark:border-zinc-800 text-zinc-300 font-mono text-xs flex items-center gap-3 shadow-inner">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {activeTopic.cards.map((card) => (
            <div
              key={card.title}
              className="group bg-white/80 dark:bg-zinc-950/70 backdrop-blur-xl border border-zinc-200/90 dark:border-zinc-800/80 rounded-[2.5rem] p-8 md:p-10 hover:border-amber-400/50 hover:shadow-[0_15px_35px_rgba(251,191,36,0.15)] hover:-translate-y-2 transition-all duration-500 shadow-xl dark:shadow-2xl relative overflow-hidden flex flex-col justify-between"
            >
              ...
              <span className="absolute -bottom-6 -right-2 text-7xl md:text-8xl font-black text-zinc-200/60 dark:text-zinc-900/40 select-none pointer-events-none group-hover:scale-110 group-hover:text-zinc-300/60 dark:group-hover:text-zinc-800/50 transition-all duration-700 font-mono uppercase tracking-tighter">
                {card.tag}
              </span>

              <div className="relative z-10">
                {/* Icon Container */}
                <div className="w-16 h-16 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl flex items-center justify-center mb-8 group-hover:border-amber-400/50 group-hover:bg-amber-400/10 transition-all shadow-sm dark:shadow-md">
                  {card.icon}
                </div>

                {/* Card Title */}
                <h3 className="text-2xl font-black text-zinc-900 dark:text-white uppercase tracking-wider mb-4 group-hover:text-amber-600 dark:group-hover:text-amber-300 transition-colors">
                  {card.title}
                </h3>

                {/* Card Description */}
                <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed font-light mb-6 text-sm md:text-base transition-colors">
                  {card.desc}
                </p>

                {/* Key Bullet Points */}
                <ul className="space-y-2.5 pt-4 border-t border-zinc-200 dark:border-zinc-900 transition-colors">
                  {card.bullets.map((bullet, bIdx) => (
                    <li key={bIdx} className="flex items-start gap-2.5 text-xs text-zinc-600 dark:text-zinc-400 transition-colors">
